...
class WCSESPHSolver(SPHSolver):
    ...
    @ti.kernel
    def init_value(self):
        # * select which para to be colored
    ...
    def advect_density(self):
        for p_i in range(self.ps.particle_num[None]):
            ...
            if self.ps.density[p_i] > self.density_0 * 1.25:    # * when the density increases sharply, will print error info.
    ...
    def substep_SympEuler(self):
        # self.compute_densities()  # * calculate density with the direct WCSPH way
        self.compute_d_density()    # * calculate density through the derivative
        self.advect_density()       # * update density here or before advect?
